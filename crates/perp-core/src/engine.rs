...
impl Engine {
        ...
        loop {
            ...
            match done {
                Done::Ok { summary, detail } => {
                    ...
                    // `A-3`: the board is regenerated by the engine after each
                    // closed step, not by someone remembering to ask. `A-7`
                    // makes a failure here a warning — losing a step because a
                    // board would not render is absurd.
                    if let Some(warning) = self.refresh_board() {
    ...
    /// Rewrite the progress board from the journal (`A-3`, `O-2`).
    ///
    /// Returns a warning rather than an error: artifacts are never on the
    /// critical path (`A-7`), and a harness that can lose a batch because a
    /// diagram would not draw will eventually do it at 3am.
    fn refresh_board(&self) -> Option<String> {
        let records = self.session.journal().read_all().ok()?;
        ...
        match crate::artifact::try_render(
            ...
            Ok(board) => board.write(&dir).err().map(|e| format!("board: {e}")),
